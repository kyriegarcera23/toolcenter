...
<div>
    <div class="card" style="overflow: visible;">
        <header class="modal-card-head has-background-info">
            <p class="modal-card-title has-text-white is-size-5 is-capitalized">Manage Locations</p>
            <div class="is-pulled-right">
                <button class="button is-success is-small" type="button" @click="addLocation()" style="margin-right:10px;"><plus></plus> Location</button>
                <button class="button is-danger is-small" type="button" @click="deleteMultiLocation()" :disabled="checkedRows.length == 0">Bulk <deleteOutline></deleteOutline></button>
            </div>
        </header>
        <section class="card-body">
            <div class="columns" style="flex-wrap: wrap;padding:10px;">
                ...
                <div class="column is-12">
                    ...
                    :is-row-checkable="(row) => restrictedRowIds.includes(row.location_id) === false"
                    checkable
                            ...
                            <b-table-column label="Action"  v-slot="props">
                                <div class="field has-addons">
                                    <p class="control is-marginless">
                                        <button @click="editLocation(props.row)" class="button is-light is-small">Edit</button>
                                    </p>
                                    <p class="control is-marginless">
                                        <button @click="deleteSpecLocation(props.row)" class="button is-danger is-small" >
...
import AddClientLocation from '../../components/modals/AddClientLocation.vue'
import EditClientLocation from '../../components/modals/EditClientLocation.vue'
    ...
    methods: {
        ...
        addLocation() {
            this.$buefy.modal.open({
                ...
                canCancel: ['x'],
                events: {
                    'addClientLocationResponse': value => {
        ...
        editLocation(location) {
            this.$buefy.modal.open({
                ...
                canCancel: ['x'],
                events: {
                    'editClientLocationResponse': value => {
                        var new_item = value.metadata.data;
                        new_item.client = value.additionalData;
        ...
        deleteSpecLocation(location) {
            this.$buefy.dialog.confirm({
                message: 'Do you want to delete this location?',
                type: 'is-danger',
                onConfirm: () => {
                    this.isBusyItems = true;
                    http.delete('/client-management/locations/'+location.id)
                    .then(response => {
                        response
                        this.deleteSpecLocationFilterObject(location);
                        this.$buefy.snackbar.open({
        ...
        deleteSpecLocationFilterObject(location) {
        ...
        deleteMultiLocation() {
            ...
            {
                location_ids.push(this.checkedRows[i].id);
            }
            this.$buefy.dialog.confirm({
                message: 'Do you want to delete these locations?',
                type: 'is-danger',
                onConfirm: () => {
                    this.isBusyItems = true;
                    http.delete('/client-management/locations/',{data:{ids:location_ids}})
                    .then(response => {
                        response
                        this.deleteMultiLocationFilterObject(location_ids);
                        this.$buefy.snackbar.open({
        ...
        deleteMultiLocationFilterObject(location_ids) {
            this.items = this.items.filter(obj => {
                return location_ids.includes(obj.id) === false;
            });
            this.isBusyItems = false;
        },
        ...
        //         {
        //             this.restrictedRowIds.push(this.items[i].location_id);
        //         }
        //     }
        // }
    }
