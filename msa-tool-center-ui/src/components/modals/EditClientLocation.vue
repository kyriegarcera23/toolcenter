<template>
    <div class="modal-card" style="width: 300px;overflow: visible;">
        <header class="modal-card-head has-background-success">
            <p class="modal-card-title has-text-white is-size-5">Edit Client Location</p>
        </header>
        ...
        <footer class="modal-card-foot custom">
            <div class="is-pulled-right">
                ...
                <b-button type="is-info" @click="saveLocation()" :loading="saveLoading">Save</b-button>
            </div>
        </footer>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            clients: [],
            selected_client: this.selectedLocation.Client,
            client: this.selectedLocation.Client,
            location_name: this.selectedLocation.name,
            location_description: this.selectedLocation.description,
            errors: {
                client_id: [],
                name: []
            },
    ...
    methods: {
        ...
        saveLocation() {
            ...
            if (this.client != {} && this.client.client_id != this.selectedLocation.Client.id)
            {
                selectedClient = this.client
            }
            else if (this.client != {} && this.client.client_id == this.selectedLocation.Client.id)
            {
                selectedClient = this.selectedLocation.Client
            }

            var inputs = {
                client_id: selectedClient.id ? selectedClient.id : selectedClient.client_id,
            ...
            http.post('/client-management/locations/update/'+this.selectedLocation.id,inputs)
            .then(response => {
                var result = {
                    metadata: response,
                ...
                //console.log(result)
                this.$emit('editClientLocationResponse', result);
