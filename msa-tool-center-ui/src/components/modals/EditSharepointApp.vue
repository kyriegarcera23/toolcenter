...
<div>
    <div class="modal-card" style="overflow: visible; width: 700px;height: calc(100vh - 40px);">
        <header class="modal-card-head has-background-success">
            <p class="modal-card-title has-text-white is-size-5 is-capitalized">Edit SharePoint Group</p>
        </header>
        <section class="modal-card-body">
            <b-tabs v-model="activeTab" expanded>
                <b-tab-item label="Group Credentials" v-if="parent.canDo('update-sharepoint-group')">
                    <b-field >
                ...
                <b-tab-item label="SharePoint Directories" v-if="parent.canDo('update-sharepoint-group') || parent.canDo('update-sharepoint-group-directories')">
        ...
        <footer class="modal-card-foot custom">
            <div class="is-pulled-right">
                <button class="button is-success" type="button" @click="updateSharepointApp()">Update</button>
            </div>
        </footer>
    </div>
</div>
...
import http from '../../http_common';
    ...
    export default {
        ...
        mounted() {
            ...
            this.initSharepointAppData();
        },
        methods: {
            initSharepointAppData: async function() {
                this.sharepointAppData = this.$parent.$parent.selectedSharepointApp;
                this.sharepointDirectories = JSON.parse(this.sharepointAppData.directories)
            ...
            updateSharepointApp: async function() {
                let s = {
                    status: true,
                    message: ''
                };
                ...
                else
                {
                    ...
                    if(!pattern.test(this.sharepointAppData.tenant_id) || !pattern.test(this.sharepointAppData.client_id) || !pattern.test(this.sharepointAppData.resource)){
                        ...
                        if (s.status)
                        {
                            http.post(`sharepoint/${this.sharepointAppData.id}`,{
                            ...
                            .then(response => {
                                this.$parent.close();
                                this.$emit('updateSharepointAppResponse', response);
                            })
                            .catch(e => {
