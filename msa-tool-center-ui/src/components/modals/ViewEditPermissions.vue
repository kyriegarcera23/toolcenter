                         ...
                         <b-field grouped>
                            <b-tabs
                                ...
                                <b-tab-item href="#Permission">
                                    ...
                                    <div class="is-ancestor is-flex-wrap" v-if="permissions.length">
                                        <b-checkbox  class="is-3 box tile" v-model="user.permissions"
                                         v-for="permission in permissions" :key="permission.id"
                                            :native-value="permission.id"
                                            :disabled="user.role.permissions.find(p => p === permission.id) ? true : false">
                                            {{ permission.name }}
                                        </b-checkbox> -->
                                    <!-- </div> -->
                                 </b-tab-item>
                                ...
                                <b-tab-item href="#Client">
                                    ...
                                    <div class="is-ancestor " v-if="clients.length">
                                        <b-checkbox  class="is-3 box tile" v-model="newClients"
                                         v-for="client in clients" :key="client.id"
                                            :native-value="client.id" true>
                                            {{ client.name }}
                                        </b-checkbox>
                                    </div>
...
export default {
    props: ['roles', 'permissions','type','clients','accounts','user_data'],
    data() {
        return {
            active_index: 0,
    ...
    created() {
        ...
        if(this.type == 'update') {
            ...
            for (let i in this.user.role.Permissions){
                permissionArr.push(this.user.role.Permissions[i].id)
            }

            this.user.role.permissions = permissionArr


            var clientArr = [];

            for (let i in this.user_data.UserClients){
                clientArr.push(this.user_data.UserClients[i].client_id)
                
            }

            this.newClients = clientArr
            

            
            // delete this.user.role.Permissions
        } 
        this.fetchData();
    },
    methods: {
